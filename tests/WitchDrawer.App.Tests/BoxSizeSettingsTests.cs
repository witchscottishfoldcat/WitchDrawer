...
public sealed class BoxSizeSettingsTests
{
    ...
    public async Task DesktopBox_DragRefreshPreservesUnchangedIcons(DrawerItemSortMode sortMode)
    {
        ...
        try
        {
            ...
            var previewNotifications = new List<string?>();
            viewModel.PropertyChanged += (_, args) => previewNotifications.Add(args.PropertyName);
            viewModel.ShowDragPreview(2, 0);
            var expandedWidth = viewModel.GridCanvasWidth;
            previewNotifications.Clear();
            viewModel.ShowDragPreview(2, 0);
            Assert.Empty(previewNotifications);
            viewModel.ShowDragPreview(3, 0);
            Assert.True(viewModel.GridCanvasWidth > expandedWidth);
            viewModel.HideDragPreview();
            Assert.True(viewModel.GridCanvasWidth < expandedWidth);
            previewNotifications.Clear();
            viewModel.HideDragPreview();
            Assert.Empty(previewNotifications);
            viewModel.ShowDragPreviewAt(10, 20, 30, 40);
            previewNotifications.Clear();
            viewModel.ShowDragPreviewAt(10, 20, 30, 40);
            Assert.Empty(previewNotifications);
            viewModel.ShowDragPreviewAt(20, 20, 30, 40);
            Assert.Equal(20, viewModel.DragPreviewLeft);
            viewModel.HideDragPreview();
